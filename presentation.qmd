---
title: "Analyse Statistique des Diplômés"
subtitle: "Enseignement Supérieur Public Tunisien"
...
**Comprendre la production de diplômés en Tunisie**
:::

::: {.fragment style="color: white; font-size: 1.3em;"}
Analyse par domaines, spécialités et établissements
:::

::: {.fragment style="color: white; font-size: 1.2em;"}
📊 55,670 diplômés | 14 universités | 22 domaines
:::

# Problématique {data-background="#0f172a"}

## Question de recherche {.smaller}

::: {.incremental}
- Comment se **répartissent** les diplômés de l'enseignement supérieur public en Tunisie ?
- Quels **déséquilibres** peut-on identifier selon :
  - Les domaines d'étude
  - Les établissements
  - Le genre (F/M)
- Quelles **structures** et **tendances** émergent des données ?
:::

## Architecture des données {.smaller}
...
%%| fig-height: 6
graph TB
    A[fact_diplomes<br/>Table principale] --> B[uni_code<br/>Universités]
    A --> C[etablissement_code<br/>Établissements]
    A --> D[domaines_code<br/>Domaines]
    A --> E[diplomes_code<br/>Types de diplômes]
...
- **Jeu de données :** Les étudiants diplômés des universités
- **Année académique :** 2021-2022
- **Date de création :** 18 Février 2024
- **Dernière modification :** 5 Mars 2024
- **Organisme :** Ministère de l'Enseignement Supérieur et de la Recherche Scientifique (Tunisie)
:::
...
stats_globales <- data.frame(
  Indicateur = c("Total diplômés", "Hommes", "Femmes", "Ratio F/H"),
  Valeur = c(
...
stats_entites <- data.frame(
  Entité = c("Universités", "Établissements", "Domaines", "Types de diplômes"),
  Nombre = c(
...
## Explorateur de Données Interactif {.smaller .scrollable}

::: {.callout-note icon="false"}
## 📊 Tableau filtrable et interactif

Explorez l'ensemble des données : filtrez par domaine, établissement, diplôme...
:::

```{r}
#| echo: false
#| out.width: "100%"
# Préparer les données pour le tableau interactif
...
# Créer le tableau interactif avec DT - version simplifiée
datatable(
  tableau_interactif,
  filter = 'top',
  options = list(
...
::: {.fragment style="color: white; font-size: 1.2em;"}
**Questions :** Quel est le déséquilibre F/M ? Quels domaines sont les plus genrés ?
:::
...
plot_ly(genre_data, 
        ...
        hoverinfo = 'text',
        text = ~paste(Genre, '<br>',
                     format(Effectif, big.mark = " "), 'diplômés<br>',
                     Pourcentage, '%')) %>%
  layout(title = list(text = "Répartition Globale par Genre", 
...
## Domaines les plus genrés {.smaller .scrollable}
...
p <- ggplot(top15_long, aes(x = reorder(domaine_nom, Effectif), y = Effectif, fill = Genre,
                            text = paste0(domaine_nom, "<br>",
                                         Genre, ": ", format(Effectif, big.mark = " "), " diplômés"))) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_manual(values = c("Hommes" = "#3b82f6", "Femmes" = "#ec4899")) +
  labs(
    title = "Répartition H/F par Domaine (Top 15)",
    x = NULL,
...
- **Barres** : Effectifs décroissants
- **Courbe cumulative** : % cumulé de 0% à 100%

**Principe 80/20** : Identifier les éléments les plus importants
- Si 20% des domaines produisent 80% des diplômés → **Forte concentration**

**Dans notre cas :**
- Les **5 premiers domaines** : ~60% des diplômés
- Les **10 premiers domaines** : ~85% des diplômés

✅ **Utilité** : Identifier où concentrer les efforts et détecter les risques de surproduction
:::
...
**Lecture du tableau :**
- **Domaine** : Nom du domaine d'étude
- **Total** : Nombre total de diplômés
- **%** : Part de ce domaine dans la production totale
- **% Cumulé** : Somme des % depuis le 1er domaine (atteint 100% au dernier)
:::
...
ggplot(top15_pareto, aes(x = reorder(domaine_nom, -Total_diplomes))) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~. * 100 / max(top15_pareto$Total_diplomes), name = "% Cumulé")
  ) +
  labs(
    title = "Diagramme de Pareto : Concentration de la Production Académique",
    x = NULL
  ) +
...
::: {.fragment style="color: white; font-size: 1.2em;"}
**Questions :** Quels établissements produisent le plus ? Sont-ils spécialisés ou polyvalents ?
:::

## Top 20 Établissements {.smaller .scrollable}
...
gt(top20_etab %>% select(Rang, etablissement_nom, Total_diplomes, Nb_domaines, Nb_specialites)) %>%
  cols_label(
    ...
    Total_diplomes = "Diplômés",
    Nb_domaines = "Domaines",
    Nb_specialites = "Spécialités"
  ) %>%
  tab_options(
...
::: {.fragment style="color: white; font-size: 1.2em;"}
**Questions :** Quelle est la structure des diplômes par domaine ? Licence, Master, Doctorat ?
:::

## Analyse croisée {.smaller}

::: {.callout-tip icon="false"}
## 🔍 Qu'est-ce qu'un tableau de contingence ?

**Tableau croisé** = Croisement de 2 variables catégorielles

**Colonnes de notre tableau :**
- **Type de diplôme** : Licence, Master professionnel, Ingénieur, Doctorat, etc.
- **Total** : Nombre de diplômés pour ce type
...
- Comprendre la structure académique de chaque domaine

**Comment lire ?**  
Pour chaque domaine, on voit les 3 types de diplômes les plus fréquents avec leur pourcentage.
:::
...
- Transformation de variables corrélées en variables non corrélées (composantes principales)
...
::: {.callout-warning icon="false"}
## ⚠️ Décalage Offre/Demande

**Mismatch académique** : Déséquilibre entre la production de diplômés et les besoins du marché du travail
:::
...
- Sous-représentation de domaines émergents
...
::: {.fragment}
✅ Interprétation : **`r ifelse(indice_herfindahl > 0.15, "FORTE concentration", ifelse(indice_herfindahl > 0.10, "Concentration MODÉRÉE", "Faible concentration"))`**
:::
:::
:::
...
- Les **`r nrow(domaines_risque)` premiers domaines** = **`r round(sum(domaines_risque$Pct), 1)`%** des diplômés
- Risque de saturation
- Diversification à renforcer
:::
...
- Études sectorielles nécessaires
- Réorientation vers domaines en demande
:::
...
- Promouvoir filières sous-représentées
- Orientation vers métiers d'avenir
:::
:::

# Synthèse Générale {data-background-gradient="linear-gradient(135deg, #667eea 0%, #764ba2 100%)"}

## Principaux résultats {.smaller}

::: {.incremental}
1. **Déséquilibre de genre** : Ratio F/H de 2.33:1, domaines fortement genrés
2. **Concentration importante** : Top 10 domaines = 85% de la production
3. **Établissements hétérogènes** : Profils variés (spécialisés vs polyvalents)
4. **Structure des diplômes** : Distribution inégale selon domaines
5. **Profils distincts** : 4 clusters de domaines identifiés par ACP
...
::: {.columns}
::: {.column width="50%"}
### Pour les décideurs 🏛️

- Ajuster l'offre de formation
- Équilibrer la production
...
- Données ponctuelles (pas d'évolution temporelle)
- Pas d'indicateurs d'employabilité
- Absence de données sur le marché du travail
:::
...
- Croisement avec données d'emploi
- Étude de l'insertion professionnelle
- Modèles prédictifs
:::
...
2. **Tableaux croisés** : Contingence entre variables catégorielles
3. **ACP** : Réduction dimensionnalité, identification profils
...
- **dplyr/tidyr** : Manipulation données
- **ggplot2/plotly** : Visualisations
- **Quarto** : Présentation interactive

---

## Bibliographie {.smaller}

- Données : Ministère de l'Enseignement Supérieur et de la Recherche Scientifique (Tunisie)
- Méthodes ACP : Husson, F., Lê, S., & Pagès, J. (2017). *Exploratory Multivariate Analysis by Example Using R*
