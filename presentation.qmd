...
```{r}
#| echo: false
top_masc <- tail(genre_domaine %>% select(domaine_nom, Hommes, Femmes, Pct_Femmes) %>% 
                   arrange(desc(Pct_Femmes)), 10)
...
ggplot(top15_long, aes(x = reorder(domaine_nom, Effectif), y = Effectif, fill = Genre)) +
...
**Principe 80/20** : Une minorité de domaines produit la majorité des diplômés
...
gt(top10_prod) %>%
  ...
  fmt_number(columns = c(Pct, Pct_cumule), decimals = 2) %>%
  opt_row_striping()
```
...
Croisement de deux variables catégorielles pour analyser la structure des diplômes par domaine
:::
